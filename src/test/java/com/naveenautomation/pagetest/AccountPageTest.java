...
public class AccountPageTest extends TestBase {
	...
	public void validateSubscribeOption() {
		newsletterPage = (NewsletterPage) new SideNavBar(wd, false)
				.OpenPageByClickOnSideNavBar(RightNavigationBar.NEWSLETTER);
	...
	public void validateRegistrationWithMultipleUsers(String fName, String lName, String email, String telephone,
		...
		accountCreatedSuccessPage = registerAcctPage.enterUserDetails(fName, lName, email, telephone, password,
				confirmPassword);
		Assert.assertEquals(accountCreatedSuccessPage.getAccountCreatedBanner(), "Your Account Has Been Created!",
	 ...
	 * (MacDesktopPage) new
	 * TopNavBar(wd,false).OpenPageByClickOnSideNavBar(TopNavigationBar.DESKTOPS);
	 * macDesktopPage.addProductToWishList(); monitorsPage =
	 * macDesktopPage.navigateToMonitorsPage(); wishListPage =
	 * monitorsPage.addMonitorsToWishList(); accountPage =
	 * wishListPage.getDataFromWishListTable();
	 * Assert.assertEquals(accountPage.myAccountText(), "My Account",
	 * "Items not added to the wishlist"); }
	 */
	...
	public void validateIfUserCanCheckOrderHistory() {
		orderHistoryPage = (OrderHistoryPage) new SideNavBar(wd, false)
				.OpenPageByClickOnSideNavBar(RightNavigationBar.ORDERHISTORY);
		Assert.assertEquals(orderHistoryPage.getOrderHistoryBanner(), "Order History",
				"User not able to check the history");
	...
	public void validateIfUserCanViewTheOrderPlaced() {
		...
		Assert.assertEquals(orderInfoPage.getOrderHistoryText(), "Order History",
				"User not able to see previous orders");
	...
	public void validateIfUserCanSeeDownloads() {
		downloadPage = (DownloadPage) new SideNavBar(wd, false)
				.OpenPageByClickOnSideNavBar(RightNavigationBar.DOWNLOADS);
		Assert.assertEquals(downloadPage.getDownloadBannerText(), "Account Downloads",
				"User not able to see downloads");
	...
	public void validateIfUserCanSeeTransactions() {
		transactionPage = (TransactionPage) new SideNavBar(wd, false)
				.OpenPageByClickOnSideNavBar(RightNavigationBar.TRANSACTIONS);
		Assert.assertEquals(transactionPage.getPaymentBannerText(), "Your Transactions",
				"User not able to see transactions");
	...
	public void validateIfUserCanSeeRewardPoints() {
		rewardPage = (RewardPage) new SideNavBar(wd, false)
				.OpenPageByClickOnSideNavBar(RightNavigationBar.REWARDPOINTS);
		Assert.assertEquals(rewardPage.getRewardsBannerText(), "Your Reward Points",
				"User not able to see reward points");
	...
	public void validateIfUserCanSeeRecurringPayments() {
		paymentsPage = (RecurringPaymentsPage) new SideNavBar(wd, false)
				.OpenPageByClickOnSideNavBar(RightNavigationBar.RECURRINGPAYMENTS);
		Assert.assertEquals(transactionPage.getPaymentBannerText(), "Recurring Payments",
				"User not able to see transactions");
